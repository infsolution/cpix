...
export const register = async (userData: FormSigninParams):Promise<IAuthenticateResponse>=>{
    const userForm ={
        name: userData.name,
    ...
    const {data}= await cPixApi.post<IAuthenticateResponse>('auth/register', userForm);
    return data;
}